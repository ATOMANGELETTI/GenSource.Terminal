...
- Terminal workspace chrome: the drag-resizable left side panel (default ~260px; toggle from a thin bottom status bar) has four equal bottom tabs, with Tab 1 as a theme-aware Windows Files explorer (all drives visible, `C:\` expanded initially, lazy tree, toolbar, file-type icons, context menu/About); the terminal tab strip starts to the right of the panel, and resizing the panel must resize the terminal with it.
