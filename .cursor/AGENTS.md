...
- Config/settings UI should write through to `other/configs/` while keeping manual edits + watchers working; avoid full UI remount/flash on save when possible, and if a refresh is unavoidable restore the active side-panel tab/state instead of jumping back to Files; Config category sub-pages use distinct icon nav (not the bottom panel tab style) and the settings content should stay as polished as the rest of the app (keep the rail); do not put agent settings in Config — Agents uses Chat, Previous chats, and Agent settings as the third rail icon; agent replies belong in the Agents panel, not the open terminal, unless the user asks for terminal work; after allow/deny of a terminal action, keep the confirmation bubble in chat and show the decision.
...
- Runtime app config lives in `other/configs/` (`appinfo.json`, `settings.json`, `keybindings.json`, `logging.json`, `agent.json`) and ships beside the installed `.exe` under `other/`; keep those JSON files comment-free and document options in `other/configs/README.md` (loader still accepts JSONC); Gemini keys live in the Stronghold vault (never the webview or shipped `agent.json`) with models Gemini 3.6 Flash, 3.5 Flash, 3.5 Flash-Lite, and 3.1 Pro; always strip `apiKey` from `agent.json` (shipped default empty); optional `vaultPassword` may be saved to `agent.json` for end users; in `tauri::is_dev` only, load gitignored `.env` names `GENSOURCE_VAULT_PASSWORD` and optional `GEMINI_API_KEY` (no `VITE_` prefix) and auto-unlock from `.env` then `agent.json` `vaultPassword` (packaged builds ignore `.env`); the Config side-panel tab edits settings via `save_settings` / `save_logging` / `save_keybindings` (and `get_logging`) while files remain manually editable; `fileIconSet` selects explorer icons (`catppuccin` / `material` / `nord`); `particleEffect` selects the hot-reloaded `dust`, `constellation`, or `orbs` terminal backdrop; `autostart` drives `@tauri-apps/plugin-autostart`; prefer `appinfo.json` visible but read-only when the platform allows.
- Opaque app-managed persistence uses `@tauri-apps/plugin-store` via `src/app/lib/app-store.ts` (AppData `app-state.json`); do not route `other/configs/` through the store; only pinned terminal tabs persist across restarts (custom names + history/scrollback); sanitize pinned scrollback so blank and prompt-only lines are not saved or restored; unpinned tabs are session-only; SCM folder path for Source Control also persists via the store; agent chat history uses portable SQLite via Rust `sqlx` at `other/database/sqlite/agents/chats/chats.db` (`tauri-plugin-sql` stays registered but maps to AppData, so frontend chat I/O is typed `invoke()`, not `Database.load()`); Stronghold salt+vault live under `other/database/stronghold/` (`salt.txt`, `vault.hold`), not AppData, and stay portable beside the exe.
...
- Terminal workspace chrome: the drag-resizable left side panel (default ~260px; toggle from a thin bottom status bar) has four equal bottom tabs — Files (theme-aware explorer: all drives, `C:\` expanded initially, lazy tree, toolbar, `fileIconSet` icons, Open in Terminal / Open in Git panel / About), Source Control (VS Code-like git via pure-Rust gitoxide/`gix`; open/close repo; empty-state open folder + red Close when a repo is open; when a repo is open, Agents-style icon rail persisted in `sessionStorage` `gensource.scm.view`: Changes lists split into Staged / Unstaged / Changes(untracked), plus a lazy worktree Git tree via `git_list_dir` with status badges, ignored paths in muted gray, and `.git` hidden; `notify` watch `git_watch_start`/`git_watch_stop` + `scm-changed` auto-stages unstaged/untracked, expanding directory paths to files and skipping one pass after Unstage; click a changed file opens a unified git-diff tab beside terminal tabs via `imara-diff`, unchanged / green add / red delete), Agents (BotIcon; Gemini via `rig-core`; chat-first in the panel with confirm-before-terminal-write; icon sub-nav like Config: Chat, Previous chats, Agent settings), and Config (category sub-pages writing to `other/configs/`); the terminal tab strip starts to the right of the panel; the status bar shows right-aligned CPU/GPU/RAM/network metrics with expanded hover tooltips and a far-right usage↔temperature toggle (temps via LibreHardwareMonitor/`lhm-sys` with NVML/WMI fallbacks; LHM builds need .NET SDK 8 and may prompt for admin); resizing the panel must resize the terminal with it.
- Runtime app logs belong under `other/logging/app/`, build logs under `other/logging/build/`, agent logs under `other/logging/agent/`; `other/configs/logging.json` has nested `app` / `build` / `agent` level toggles (agent also has `prompts` / `replies` / `tools` / `reasoning`); log files named `[TIME]_[DATE]_[APPVERSION].log`; user-runnable maintenance bats live in `other/utilities/scripts/` (`archive-logs.bat` via `other/utilities/7zr.exe` → timestamped `.7z` in `scripts/archive/`, plus `clean-logs-app.bat`, `clean-logs-build.bat`, `clean-logs-agent.bat`, `clean-logs-archive.bat`).
